...
describe('sSE', () => {
  ...
  it('should forward decoder tail when a multibyte UTF-8 character ends the stream', async () => {
    const encoder = new TextEncoder();
    const bytes = encoder.encode('data: {"text":"已完成"}\n\n');
    ...
    expect(messages.join('')).toBe('data: {"text":"已完成"}\n\n');
  });
