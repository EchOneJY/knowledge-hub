...
// 搜索表单容器的底部留白:分隔条是绝对定位的装饰带,pb 需同时容纳「内容与分隔条的间距 + 分隔条高度 + 分隔条下方间隙」。
// 存在工具栏时分隔条贴容器底边,pb 相应收紧,避免工具栏上方多出一段留白;
// 没有工具栏时保留分隔条下方的间隙,否则表头会紧贴分隔条。
const formPaddingBottomClass = computed(() => {
  if (!isCompactForm.value) {
    return isSeparator.value ? 'pb-4' : 'pb-0';
  }
  if (!isSeparator.value) {
    return 'pb-4';
  }
  return showToolbar.value ? 'pb-5 md:pb-4' : 'pb-6';
});
    ...
    >
      ...
      <template #form>
        <div
          ...
          :class="cn('relative rounded-sm py-1', formPaddingBottomClass)"
        >
          <slot name="form">
            <Form>
              <template
          ...
          <div
            ...
            :class="
              cn(
                'absolute -left-2 z-100 h-2 w-[calc(100%+1rem)] overflow-hidden bg-background-deep md:h-3',
                showToolbar ? 'bottom-0' : 'bottom-1 md:bottom-2',
              )
            "
          ></div>
        </div>
