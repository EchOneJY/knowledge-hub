  ...
  async streamChat(dto: ChatStreamDto, user: AuthUser, res: Response) {
    ...
    const stream = createUIMessageStream<KhUIMessage>({
      execute: async ({ writer }) => {
        ...
        // NOTE: 记忆拼进最后的用户消息而不是 messages[0] 的 SystemMessage——
        // summarizationMiddleware 触发时会摘要并删除开头的 SystemMessage（含记忆），
        // 而最后一条消息在 keep 窗口内，永远不会被摘要掉
        const memoryText = this.longMemory.buildMemoryText(memHits);
        const finalPrompt = memoryText ? `${memoryText}\n\n${prompt}` : prompt;

        const langchainStream = await this.agent.stream(
          {
            messages: [...history, new HumanMessage(finalPrompt)],
          },
